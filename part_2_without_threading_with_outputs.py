...
def plotcode(folderpath,hsvtop,hsvphysical):
        ...
        def filter_top_of_robot(self, frame):
            ...
            if contours.__len__() != 0:
                ...
                if int(x) != 0 and int(y) != 0:
                    ...
                    y1 = y + self.adj_y
                    #print(y,x)
                    if img_plot[int(y1),int(x1),0]==255: # Check if the pixel is plotted on White Foreground or Black Background
                    ...
                    cv2.line(self.thick_plot, (int(x1), int(y1)), (int(x1), int(y1)), (0, 255, 0), 2)
            ...
            cv2.imshow("Off-set",self.thick_plot)
    ...
    for file_name in glob.glob(path + '*.mov' or '*.mp4'):
        global th_count
        th = compute_frame(file_name,th_count)
        th_count+=1
        th.run()
    return
