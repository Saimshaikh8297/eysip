...
def evaluate(csv_perfect):
    global path_to_circle,path_to_plot,path_to_perfect_image,feature_list,coordinates,result
    ...
    with open("Results/intermediate_results.csv",'r') as csvfile:
        ...
        # for i in range(0,perfect_values.__len__()):
            path_to_plot =row[1] #extracting perfect trajectory for evaluation
            print(path_to_plot)
            path_to_circle = row[2]  # extracting perfect trajectory for evaluation
            print(path_to_circle)
            check_result = programmatic(path_to_circle)
            print(check_result)
            row.append(str(check_result))
